...
인물 299 · 사건 263 · 장소 83(실제 62, 이야기 속 21) · 묶음서사 33 · 원전 13
...
`CLAUDE.md`(지침) + `집필-지침.md`(문장·수위 규칙) + `knowledge/`(색인·시대·인물·사건·장소·묶음·원전). 다 합쳐 874KB.
...
신들의 계보 130 · 파우사니아스 126 · 리비우스 67 · 히기누스 11 · 일과 날 8 · 호메로스 찬가 5.
...
sources/
  ...
  hyginus-fabulae-{1..5}-grant.{html,txt}                 theoi.com — **발췌만 들어 있다**(우화 1~4·50~55·100~103·150~155·200~203). 이설의 주 출처로 쓰지 못했다
...
- **시간축에 "순서를 모른다"를, 지도에 "장소를 모른다"를 표시할 방법이 없다.**
  전건을 [불확실-목록.md](불확실-목록.md) 에 모았다 — 같은 이야기 안에서 순서를 모르는 짝 12건,
  장소 없는 사건 19건(그중 14건은 원전이 말하지 않는 것이 맞다).
  스키마 후보 셋 가운데 **`place_unknown` 을 먼저 넣기로 정했다**(`places_variant` 는 필요 없어져 뺐다).
- **`sources/hyginus-fabulae-*.txt` 가 발췌다.** theoi.com 추출이 잘려 우화 1~4·50~55·100~103·150~155·200~203 만 들어 있다. 그래서 이설의 주 출처로 쓸 수 없었다(인용 11건에 그친다). 다시 받는 것이 필요하다.
...
3. **`place_unknown` 넣기** — 자동 점검의 장소 목록에서 "원전이 말하지 않는 것"과 "아직 안 넣은 것"을 가른다([불확실-목록.md](불확실-목록.md) D-3)
4. **히기누스 원문 다시 받기** — 지금 파일이 발췌라 이설의 주 출처로 쓸 수 없다
5. **남은 장소 다섯** — 리디아·리비아·오이칼리아를 항목으로 만들지 정한다([불확실-목록.md](불확실-목록.md) B-3)
